...
pub fn import(filepath: &str,password: &str) -> Result<Vec<OTPElement>, String> {
    ...
    let aegis_encrypted: AegisEncryptedDatabase = match serde_json::from_str(&file_to_import_contents) {
        Ok(result) => result,
    ...
    for slot in aegis_encrypted.header.slots.iter().filter(|item| item._type == 1) {
    ...
    match master_key {
        Some(master_key) => {
            ...
            let decrypted_db = match cipher.decrypt(
                Nonce::from_slice(Vec::from_hex(&aegis_encrypted.header.params.nonce).expect("Failed to parse hex nonce").as_slice()),
                [content,Vec::from_hex(&aegis_encrypted.header.params.tag).expect("Failed to parse hex tag")].concat().as_slice()
            ) {
                Ok(result) => result,
...
fn get_master_key(slot: &AegisEncryptedSlot, password: &str) -> Result<Vec<u8>,String> {
    ...
    if let Err(e) = scrypt(password.as_bytes(), salt.as_slice(), &params, output.as_mut_slice()) {
        return Err(format!("Error during scrypt key derivation: {:?}",e));
    }

    let cipher = Aes256Gcm::new(Key::from_slice(output.as_slice()));
    let cipher_text = [Vec::from_hex(&slot.key).expect("Failed to parse hex key"),Vec::from_hex(&slot.key_params.tag).expect("Failed to parse hex tag")].concat();

    match cipher.decrypt(
        Nonce::from_slice(Vec::from_hex(&slot.key_params.nonce).expect("Failed to parse hex nonce").as_slice()),
